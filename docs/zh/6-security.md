...
有多认真？源码中 **18 个文件**专门负责一个 BashTool 的安全审查。AI 想用 `rm -rf` 要经过多层审查才能执行。这就是 Anthropic 和其他 AI 编程工具的根本区别——它让 AI 直接操作你的真实环境，但用了 51 万行代码来确保这个过程的安全。
...
Claude Code 的安全不仅仅是"拦截危险操作"。它对每一个外部依赖都有对应的失败策略：

| 外部依赖 | 失败策略 |
|---------|---------|
| API 请求失败 | 指数退避重试 + 错误信息反馈给模型 |
| 工具执行超时 | 超时中断 + 结果截断 |
| 上下文溢出 | 三层渐进压缩（微压缩 → 自动压缩 → 完全压缩） |
| 连续压缩失败 | 熔断器：3 次失败后停止尝试 |
| 会话中断 | Transcript 持久化，可恢复 |
| MCP 服务不可用 | 降级为内置工具集 |

这就是为什么 Claude Code 在实际使用中很少"崩溃"——不是因为没有错误，而是每个错误都有预案。
