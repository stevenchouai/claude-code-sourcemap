...
2026 年 3 月，有人发现 Claude Code 的 npm 发布包里意外包含了 source map 文件。通过这个 source map，社区还原了约 **1884 个 TypeScript 源文件**，形成了一个可研究的源码视图。

它能帮助我们理解 Claude Code 的很多架构选择，但它**不是 Anthropic 官方源码发布**，也不应被表述为官方内部仓库结构或未公开实现说明。
...
- 为什么它更像一个本地 agent runtime，而不只是代码库 RAG？（见第 9 篇研究札记）
...
我们把 Claude Code 的架构拆成了 8 个教程章节，并补充研究札记，像导游一样带你走一遍。每章聚焦一个核心概念，用图表和类比让你理解，而不是让你硬看代码。
